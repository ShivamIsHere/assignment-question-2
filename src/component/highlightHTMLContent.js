function highlightHTMLContent(htmlContent, plainText, st,end) {
  ...
  while (i < htmlContent.length && j < plainText.length) {
      ...
      if ((plainText[j] === " " )||(htmlContent[i] === plainText[j])) {
...
plainTextPositions.map((ele)=>{
 
 
  let st=ele.start;
  let end=ele.end;
let k= (highlightHTMLContent(htmlContent, plainText,st,end));
htmlContent=k;
